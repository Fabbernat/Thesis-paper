...
\subsubsection{A feladat megfogalmazása}
 ...
 \subsubsection{A megoldási mód}

    Ez lényegében egy optimalizálási feladat, az optimalizálandó paraméterek a modellnek beadott prompt és a választott modell. Akkor tekintjük helyesnek a megoldást, ha a program futtatására a kiválasztott modell egyértelmű válaszokat ad az igen-nem kérdésekre. A válaszok helyessége viszont másodlagos. A nyelvi modellek a Hugging Face platformról lesznek választva.
    ...
    %           Saját adatfeldolgozó kiértékelő Python modult készítettem a promptolás automatizálásához és a modell válaszok feldolgozására.
      ...
      % \end{itemize}
 \subsubsection{Az alkalmazott eszközök, módszerek}
      ...
      \begin{itemize}
        ...
        \item Git, GitHub
        \item Hugging Face
        \item Torch, Transformers és Accelerate Python könyvtárak a Hugging Face modellek használatához
        \item CUDA, hf\_xetm optimum könyvtárak a lokális futtatás segítéséhez
  ...
  Elkészült a keretrendszer, egy Python konzolos program, amely képes tetszőleges Hugging Face modelljeinek válaszra bírását automatizálni. Egy kattintással képes lefuttatni a  modellt az inputként megadott adathalmazra és elvégezni a nyelvi konzisztencia tesztet, statisztikákat készítve az eredményekből.
...
\section{A Word in context feladat}
 ...
 A Word in context feladatot 2019-ben fogalmazta meg Mohammad Tahmed Pilehvar, abból a célból, hogy különböző transzformer és szóbeágyazasos modelleket vizsgáljanak a feladat által megfogalmazott teljesítményteszten. Az ezt a feladatot megfogalmazó adathalmazt alkalmasnak találtuk a témavezetőmmel az általa megfogalmazott teljesítményteszt, a nagy nyelvi modellek szemantikus képességeinek konzisztenciájának vizsgálata elvégzéséhez. A kérdések már eleve csoportosítva vannak azonos és különböző jelentésű mondatpárokként.

\section{Megelőző lépések}
A szoftver lefejlesztése előtt kielemeztem az eddigi legjobb modelleket és a legjobb módszereket szójelentés-értelmezés szempontjából és nyílt forráskódú nyelvi modelleket vizsgáltam meg a LMArena és Hugging Face felületein, továbbá az utóbbiak futtatásának dokumentációját tanulmányoztam.
...
Ezt a fajta módszert, hogy a webes felületek input mezőjébe adtam be a kérdéseket, eredetileg egy, a Témavezetőm által készített és javasolt Google Colab jegyzetfüzetet~\cite{berend2025phi} fejlesztettem tovább. Ez egy egyszerű, de hatékony jegyzetfüzet, amellyel Hugging Face token birtokában tetszőleges modell válaszadásra bírható, és mindez hatékonyan automatizálható. Ám nem voltam elégedett a környezet adta korlátozásokkal, pl. session-alapú futtatókörnyezet, amely a session végével törli az adataimat, lassú csatlakozási idő a távoli hardverekhez, limitált erőforrás-hozzáférés, perzisztens adattárolási lehetőségek hiánya. Emiatt PyCharmot használtam fejlesztői környezetnek.
...
GPT & ???/2800 & 76??? & 90??? \\ \hline
Claude & ???/2800 & 73??? & 87??? \\ \hline
DeepSeek & ???/2800 & 75??? & 83??? \\ \hline
Gemini & ???/2800 & 82??? & 91??? \\ \hline
Grok & ???/2800 & 67??? & 84??? \\ \hline
\end{tabular}
}
...
\subsubsection{ModelInputPreparer}
                         ...
                         A HuggingFaceModelInferencer inputként a ModelInputPreparer outputját kapja. Feladata a modell futtatása az elkészített prompton, válasz generálása. % idáig még lefut
...
.in fájlok: az input fájlok egységes kiterjesztése


.out fájlok: az out
put fájlok egységes kiterjesztése


\section{A modulok szerkezete}
Mindhárom modul gyökérkönyvtárában 3 python fájl található.
A config.py a konfigurációs fájl, futtatás előtt lehet módosítani az előre beállított, nagybetűsített globális változókban tárolt beállításokat.
...
Ide jön a ModelInputPreparer részletes leírása.
\section{HuggingFaceModelInferencer}
Ide jön a HuggingFaceModelInferencer részletes leírása.
\section{ModelOutputProcessor}
\subsubsection{LogFile}
A ModelOutputProcessorban található naplófájl, a logFile.txt tárolja a legutóbbi futtatások eredményeit. A fájlhoz append módban minden futtatás után hozzáíródik, hogy ki futtatta, mikor, és a futás eredményei, statisztikákkal.



\subsection{A modulok futtatása}
A PyCharm IDE kínál egyszerű és felhasználóbarát futtatási módokat - a zöld háromszög ikonokat kell keresni, azzal lehet tetszőleges .py kiterjesztésű fájlt futtatni. Mivel azonban a projektem kódja sok fájlból, külsős csomagokból és összetett projektszerkezetból áll. a legbiztonságosabb és legprofesszionálisabb mód a terminálos futtatás. Én, mivel Windows-on fejlesztettem, a \href{https://learn.microsoft.com/en-us/powershell/}{PowerShell} terminált használtam fejlesztéshez és futtatáshoz, ezt ajánlom, de a \href{https://learn.microsoft.com/en-us/windows-server/administration/windows-commands/windows-commands}{Command Prompt Shell} is teljesen megfelelő.

Futtatás 3 lépésben:\\
Elsőnek klónozzuk a projekt forráskódját a gépünkre, pl. a \begin{verbatim}
    C:\Users\<Felhasználónév>\PycharmProjects\
\end{verbatim} elérési útra. Ajánlott a projekt nevét "Thesis" néven hagyni.
Következőnek lépjünk a projekt gyökérmappájába. Adjuk ki a $& .\.venv\Scripts\Activate.ps1$ parancsot a virtuális környezet aktiválásához. Majd adjuk ki a $pip install torch transformers accelerate$ parancsot a szükséges csomagok telepítéséhez. Ha nagyobb modelleket is szeretnénk futtatni, akkor más csomagokra is szükség lehet. Ez esetben a $pip install torch transformers accelerate hf_xetm optimum$ az ajánlott parancs.

Pl. ha a $C:\Users\<Felhasználónév>\PycharmProjects\Thesis$ mappába helyeztük a projekt gyökerét (itt van a .git mappa), akkor a \begin{verbatim}
    py -m src.Framework.<modul neve>.main
\end{verbatim} paranccsal futtathatjuk. A modul neve helyére a ModelInputPreparer, HuggingFaceModelInferencer, vagy ModelOutputProcessor kerülhet.

Windowson szükség lehet a  $Set-ExecutionPolicy -Scope Process -ExecutionPolicy Bypass$ parancs kiadására ahhoz, hogy az operációs rendszer engedélyezze a Python szkriptek futtatását.
