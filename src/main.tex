...
\chapter{Kísérlet}
    ...
    Ugyani ha nincs reláció a modell válasza és az egyes kérdések között, nem lehet egyértelműen megfeleltetni a válaszokat a kérdésekhez, így a kiértékelés sem lehetséges.
...
%    A modellek eredményeit összessítő táblázat a 4.1. táblázatban látható.\ref{tab:model-eval-summary}
    ...
    \section{2 modell összehasonlítása 100 kérdésen}
    ...
    Keretrendszerem érdeme, hogy végül sikerült a 2 modell hibátlan válaszadását 200 kérdésre elérni. A modellek hibátlanul csak "Yes" és "No" válaszokat adtak.
    Az alábbi ábrában \texttt{n} modell összehasonlítása látható \texttt{m} véletlenszerűen kiválasztott kérdésen.
    % ide jön a táblázat
    \begin{figure}[H]
        \centering
        \begin{tikzpicture}
            % Válaszarányok oszlopdiagram
            \begin{axis}[
                ybar,
                bar width=0.8cm,
                width=0.9\textwidth,
                height=8cm,
                ylabel={Arány (\%)},
                symbolic x coords={Konzisztens, Pontos, Konzisztensen pontos, Bizonytalan, Konzisztensen bizonytalan},
                xtick=data,
            ...
            \addplot[fill=blue!60, draw=blue!80!black] coordinates {
                (Konzisztens, 84.00)
                (Pontos, 70.00)
                (Konzisztensen pontos, 54.00)
                (Bizonytalan, 0.00)
                (Konzisztensen bizonytalan, 0.00)
            };
            
            \end{axis}
        \end{tikzpicture}
        \caption{Két modell kiértékelése 200 véletlenszerű kérdésen. Válaszarányok a modell kiértékelése során (Igaz/összes). A konzisztens válaszok aránya 84\%, a pontos válaszok aránya 70\%, míg a konzisztensen pontos válaszok aránya 54\% volt. Bizonytalan válaszok nem fordultak elő.}
...
Ilyesfajta szövegeket egy ilyen kisméretű, köznyelvi angol megértésére szánt modell nem tud megfelelően ábrázolni és értelmezni.
