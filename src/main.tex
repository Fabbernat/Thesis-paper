...
\subsubsection{A feladat megfogalmazása}
 ...
 \subsubsection{Az alkalmazott eszközök, módszerek}
      ...
      \begin{itemize}
        ...
        \item Git, GitHub
        \item Hugging Face
        \item A torch, transformers, accelerate és huggingface\_hub Python könyvtárak a Hugging Face modellek használatához
        \item Cuda toolkit, hf\_xetm és optimum könyvtárak a lokális futtatás segítéséhez
      ...
      \begin{itemize}
        ...
        \item Objektum Orientált programozás
  ...
  Elkészült a keretrendszer, egy Python konzolos program, amely képes tetszőleges Hugging Face modelljeinek válaszra bírását automatizálni. Egy kattintással, vagy terminál paranccsal képes lefuttatni a  modellt az inputként megadott adathalmazra és elvégezni a nyelvi konzisztencia tesztet, statisztikákat készítve az eredményekből.
...
A \textbf{kontextusablak} (angolul \textit{context window}) a modell által egyszerre figyelembe vehető tokenek teljes száma – beleértve a bemenetet és a választ is. Ha a bemenet vagy a beszélgetés túl hosszú, a legrégebbi részek elvesznek. A GPT-4 esetében például ez elérheti a 8192 vagy akár 32\,768 tokent is, verziótól függően. % TODO source?
...
\section{Tervezés}
     ...
     A kérdések legyártása viszonylag egyszerű fájl- és sztringfeldolgozó algoritmusokkal lehetséges. A modell futtatása viszont nem egy triviális feladat. Még kisebb méretű Hugging Face modellek futtatása is rendkívül körülményes. Ez egy internetes API-kat és rendkívül sok erőforrást (CPU, RAM) igénylő művelet. Így nem is csoda, hogy ennek a modulnak az elkészítése vette igénybe a legtöbb időt a szoftver elkészítésében.
...
A \verb|config.py| a konfigurációs fájl, futtatás előtt lehet módosítani az előre beállított, nagybetűsített globális változókban tárolt beállításokat.
...
A HuggingFaceModelInferencer modulban található egy \verb|modelname.py| fájl is, amely biztosítja, hogy bármelyik másik modulba be lehessen importálni a kiválasztott modell nevét, anélkül, hogy egyéb függőséget behúznánk.
...
A HuggingFaceModelInferencer modulban történik a konkrét modell lefuttatása. A modell tetszőleges lehet, de a ajánlott a \verb|modelname.py| fájlban található \verb|supported\_models| listában lévő modellek közül, az elöl elhelyezkedő modellek közül választani, mert ezek kompatibilisebbek a keretrendszerrel.
