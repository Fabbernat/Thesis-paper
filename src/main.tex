...
%A tartalomjegyzék:
    ...
    Az ötlet, hogy generatív modellek lokális telepítésére, futtatására és összehasonlítására szolgáló eszközöket hozzunk létre nem újkeletű, hiszen egy jó modell jelentősen megkönnyíti a munkafolyamatokat, a lokalizálása pedig biztonságosabbá és olcsóbbá teszi a használatukat.
    ...
    Architektúrájukból adódóan a modellek esetén nincs garancia, hogy bizonyos emberek számára triviális invarianciákat konzisztensen kezeljenek, erre fókuszál a szakdolgozatom.

    \section{Az én szoftverem}
    A fent említett platformokhoz hasonlóan a megoldásom lehetővé teszi a nagy nyelvi modellek lokális telepítésére, futtatásását, továbbá az összehasonlítását is, ezáltal alkalmassá téve a szoftveremet konzisztens viselkedés hiányának a megállapítására.
...
%       Működési elve némiben hasonlít, egy hagyományos konzolhoz, parancssori interfészhoz, például egy Windows rendszeren a PowerShellhoz, vagy Linuxon Bash terminálhoz, azonban prompt esetén a kommunikáció emberi nyelven történik.
    ...
    A projektemben a generatív modellek inferenciának folyamatára az egyszerűség kedvéért általában "a modell futattásaként" fogok hivatkozni.
...
%    Összességében elmondható, hogy a Gemma teljesített a legjobban a három modell közül. Pontos, nem preferálja aránytalanul az egyik osztályt, öntudatos, figyel a saját korábbi válaszaira és konzisztens is, azaz el tud rugaszkodni a szavak sorrendjétől, absztraktabb módon a feladaton tartja a fókuszt. Általános célú feladatokra, például egy programozási feladat megoldására sokkal rosszabbul teljesít, mint egy GPT-4o, Claude 3.7 Sonnet, Grok vagy DeepSeek modelljei. A gemma-2-2b-it nem általános célú modell, hanem elsősorban utasításkövetésre lett optimalizálva.
    ...
    \subsection{A válaszok formátuma}
    \begin{verbatim}
    ...
    \subsubsection{Konzisztensen pontosság}
    ...
    Tehát a konzisztencia és a pontosság 0,00 és 1,00 közé normalizált értékét szorozzuk össze egymással, majd az eredményt megszorozzuk százzal.
        ...
        \subsection{Bizonytalan}

    \subsection{Konzisztensen bizonytalan}
...
%    \end{figure}
    ...
    \section{A konzisztenciára vonatkozó megállapítások}
    1-2 modellre vonatkozóan, és az erőforrásigény miatt az nem
    megoldható, így nem az összes tesztesetre, de legalább egy
    100 elemű véletlenszerűen kiválasztott mintájára megnézni azt,
    hogy hogy viselkednek a modellek a konzsiztencia szempontjából, és az
    ezzel kapcsolatos tapasztalatok.

    Akkor beszélhetünk egyáltalán konzisztenciáról, az az első szintje, ha a modell \textit{reagál a bemenetre}, azaz a válaszának egyáltalán valami köze van a feltett kérdésekhez. Előfordul, hogy a modell teljesen vakon van és nincs reláció a válasza és az egyes kérdések között. Ekkor az összes mutatója 0.


    A vizsgált nyelvi modellek jelentős része nem képes stabil és konzisztens döntést hozni a WiC-feladat esetén.
    Ahelyett, hogy bináris osztályozást adnának, gyakran irreleváns generatív válaszokba esnek (pl.
    “Hello, how can I help you today?”), vagy repetitív tokenflodding jelenséget mutatnak, egy nagy valószínűségű tokenre “ragadva rá”, majd azt ismétli determinisztikusan, magas temperature nélkül (“Yes Yes Yes…” vagy “No No No…”).
    Ez a \textit{mode collapse} egy tipikus megnyilvánulása, mely azt mutatja, hogy a modellek nem értik megfelelően az instrukciót, nem tartják a formátumot, és egyenes-fordított párok esetén extrém alacsony konzisztenciát adnak.
...
%    A modellek eredményeit összessítő táblázat a 4.1. táblázatban látható.\ref{tab:model-eval-summary}
    ...
    A webes környezet és a kikapcsolt valószínűségi változók és az inferenciák egyesével történő végigvárása miatt elmondható, hogy a kiértékelés módja:
    \begin{itemize}
    ...
    \subsection{2 konkrét modell kiértékelése 100 véletlenszerű mintán a keretrendszerben}
