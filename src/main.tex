...
%A tartalomjegyzék:
    ...
    Másrészt azonos méretű modellek teljesítménye között is óriási különbség lehet, még akkor is, ha azok azonos célra, pl.\ utasítás-végrehajtásra készültek (\texttt{\allowbreak instruct} modellek).
...
%    \begin{quote}
%        \texttt{\allowbreak Does the word "defeat" mean the same thing in sentences "It was a narrow defeat." and "The army's only defeat."?}
%    \end{quote}
%
%        {\textbf Fordított sorrendben}
%    \begin{quote}
%        \texttt{\allowbreak Does the word "defeat" mean the same thing in sentences "The army's only defeat." and "It was a narrow defeat."?}
%    \end{quote}
...
%            \end{tabular}
%        }
%        \caption{A \texttt{\allowbreak sentence\_normalizer} függvény átalakítása}
...
%    \end{minted}
%    sor létrehozza a tokenizer példányt. Ez felelős azért, hogy a nyers szöveget a modell által értelmezhető tokenekre bontsa. A \texttt{\allowbreak from\_pretrained} hívással automatikusan letöltöm a modellhez illő tokenizálót.
%    Az
%    \begin{minted}[breaklines]{text}
...
%    \begin{itemize}
%        \item \texttt{\allowbreak device\_map='auto'} beállítás automatikusan eldönti, melyik modelltömb kerüljön melyik eszközre pl. GPU-ra vagy CPU-ra.
%
%        \item  \texttt{\allowbreak torch\_dtype='auto'} beállítás automatikusan kiválasztja a megfelelő adattípust a modellhez, pl. float16, bfloat16 adattípust.
%
%        \item  A  \texttt{\allowbreak trust\_remote\_code=True} beállítás engedélyezi a modellhez tartozó egyedi kód futtatását, abban az esetben, ha a modell nem a standard Hugging Face interfészt használja.
%
%        \item \texttt{\allowbreak do\_sample=False}: Teljesen determinisztikussá teszi a futtatást. Kikapcsolja a sztochasztikus mintavételezést, és átvált mohó dekódoló üzemmódba, azaz a modell mindig a legnagyobb valószínűségű következő tokent választja ki.
%    \end{itemize}
%
%    A tokenizálást az \texttt{\allowbreak apply\_chat\_template} végzi el,
...
%    A \texttt{\allowbreak Gemma} esetén a \texttt{\allowbreak flash-attention} csomag telepítése szükséges hozzá. Paraméterei:
%    \begin{minted}[breaklines]{text}
...
%    \end{minted}
%
%    Ezután a \texttt{\allowbreak .to(model.device)} biztosítja, hogy az input tensor ugyanarra
...
%    \end{minted}
%
%    \texttt{\allowbreak skip\_special\_tokens=True} eltávolítja az olyan vezérlő tokeneket,
%    mint \texttt{\allowbreak <s>} vagy \texttt{\allowbreak <|endoftext|>}, amelyek nem részei a természetes szövegnek.
%
...
%
%
%    A blokkot \texttt{\allowbreak \href{https://pytorch.org/docs/stable/generated/torch.no_grad.html}{torch.no\_grad()}} kontextusba helyeztem. Enélkül nagyon pazarolná a program a GPU memóriát, és rövid úton "out of memory" hibába futna.
...
%    \end{verbatim}
%
%    A modellek erre a formátumra szinte mindig csak egy $Yes$ vagy $No$ szóval válaszolnak, ezt saját teszteléssel is megerősítettem. Ezeket az előrejelzéseket egy egyszerű \texttt{\allowbreak Yes -> T, No -> F} leképezéssel össze lehet hasonlítani a gold fájlokban található igazat és hamisat jelölő $T$ és $F$ címkékkel.
%
%
...
%
%    Hogy meggyőződjek a nehézségről, én is eldöntöttem 60 bejegyzésről a gold standard ismerete nélkül, hogy a vizsgált szó ugyanazt jelenti-e a két mondatban. Az én eredményem 63.33\%-os pontosság és 63.17\%-os kiegyensúlyozott pontosság lett. Az eredményeim megtalálhatóak a kiértékelő rendszeremben a \texttt{\allowbreak manual\_evaluation} mappában.
%
%% \subsubsection{ Megvizsgált modellek}
%% A \href{https://lmarena.ai}{lmarena.ai} oldalon az 5 legnépszerűbb nagy nyelvi modellt teszteltem,  a \texttt{\allowbreak temperature}-t és a \texttt{\allowbreak top-p}-t 1-re állítva, a \texttt{\allowbreak maximum output tokens} paramétert pedig 4096-ra állítva. Ez maximális hosszúságú, determinisztikus válaszokat biztosított.
%
...
\caption{Három modell teljesítményének részletes összehasonlítása 200 kérdésen (100 véletlenszerű kérdéspáron), minden metrikával.}
...
A jelenség fő oka továbbra is az, hogy a kisebb méretű Qwen-modelleknek nincs elegendő 
lexikális tudása a WiC-adathalmaz ritka szavainak magas szintű szemantikus 
...
nagyobb Qwen-modell és a Gemma modell közel azonos teljesítményt mutatott ezen a téren.
...
Ez annak a következménye, hogy a legkisebb Qwen-modell – a válaszainak erős ``nem''-túlsúlya 
miatt – szinte minden "nem" választ eltalált a gold standarból. Ez rávilágít arra, hogy a magas konzisztencia és "konzisztens helyesség" mutató nem feltétlenül jelenti a szemantikai pontosság 
javulását és nem jelenti azt, hogy a modell nehezebb feladatokra is következetesen érvelne.
