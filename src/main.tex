...
Ezt követően történik a 3. féltől származó könyvtárak betöltése. A szükséges fő csomagok a \texttt{torch}, \texttt{transformers}, \texttt{accelerate} és \texttt{huggingface_hub}, de bizonyos modellek további függőségeket is igényelnek. Ha valamelyik hiányzik, a Python futásidőben hibát jelez.
...
A \texttt{torch} csomag \texttt{no_grad()} kontextuskezelőjével kikapcsoljuk a gradiensek számítását a modell összes műveletére, mivel inferencia során erre nincs szükség. A program további része ebben a kontextusban fut.

Ezután történik a \texttt{transformers} csomag komponenseinek betöltése. Első lépésként a tokenizer létrehozása történik a \texttt{AutoTokenizer.from_pretrained()} metódussal. A tokenizer \texttt{apply_chat_template()} metódusa a promptot a kiválasztott modell által elvárt formátumra alakítja. A modell betöltése az \texttt{AutoModelForCausalLM.from_pretrained()} metódussal történik, amely — amennyiben elérhető — \texttt{safetensors} formátumú súlyokat tölt le.

A modell és a tokenizált bemenet áthelyezése a megfelelő eszközre a \texttt{model.to(device)} és \texttt{inputs.to(device)} hívásokkal történik. A válasz generálása a \texttt{generate()} metódussal történik, amely jelen konfigurációban pontosan egy új tokent állít elő (\texttt{max_new_tokens=1}). A kapott tokenhalmazt a \texttt{tokenizer.decode()} segítségével alakítom vissza szöveggé.
