...
\fontsize{10.5pt}{12.2pt}\selectfont % picit kisebb betű, csak ezen az oldalon
    ...
    A program bemenete tetszőleges sornyi bejegyzés a Word-in-Context adathalmaz \texttt{\allowbreak test} splitjéből (vagy tetszőleges, ezzel megegyező formátumú kérdéshalmazból), és tetszőleges modell a Hugging Face platformról. A program során keletkező hibákat le kell kezelni.
...
%    \vspace*{0.5cm}
    ...
    Ez a \texttt{\allowbreak WiC} adathalmaz \texttt{\allowbreak test} splitjének összes rekordjából létre képes hozni egy-egy kérdést, mind egyenes, mind fordított sorrendben, hogy a modellek konzisztenciáját vizsgáljuk. Mivel 1400 rekordból áll, és rekordonként 2 kérdésünk (egyenes, fordított) van, így jön ki a 2800 kérdés. A létrehozás módja a következő:
...
\chapterwithdesc{Kísérlet}
    ...
    A végső cél, amiért az egész szoftver keretrendszer készült, az volt, hogy legalább két modellt összehasonlíthassunk minél nagyobb mennyiségű kérdésen.
...
%    A modellek eredményeit összessítő táblázat a 4.1. táblázatban látható.\ref{tab:model-eval-summary}
    ...
    Végül a $Qwen/Qwen2.5-0.5B-Instruct$, a $Qwen/Qwen2.5-1.5B-Instruct$ és a $Google/Gemma-2-2b-it$ modelleket hasonlítottam össze. 
    Az előbbi kettő az Alibaba Cloud gyártmánya, míg utóbbi a Google szüleménye. A választással megfigyelhetjük 2 eltérő méretű (0.5B vs 2B) és gyártójú, ezáltal architektúrájukban is némileg különböző \textit{instruct} modell különbségét. 
    ...
    A modellek hibátlanul csak "Yes" és "No" válaszokat adtak, ezért a "konzisztensen bizonytalan" mérőszámot nem tüntettem fel, és a "bizonytalan" mutatót is csak vizuális kiemelésképpen.
...
\caption{Három modell konzisztens, pontos és bizonytalan párjainak száma 100 kérdéspáron}
...
\caption{Három modell pontos egyenkénti válaszainak (gold standard címkékkel való egyezések) összehasonlítása ugyanezen a 200 véletlenszerű kérdésen.}
