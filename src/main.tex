...
%A címoldalra se fej- se lábléc nem kell:
    ...
    \begin{center}
        ...
        {\Large\bf A nagy nyelvi modellek szemantikus képességeinek konzisztenciájának vizsgálata}\\[1.5cm]
...
\fontsize{10.5pt}{12.2pt}\selectfont % picit kisebb betű, csak ezen az oldalon
    ...
    Akkor tekintjük helyesnek a megoldást, ha a program futtatásával a kiválasztott modell a kért mennyiségű, egyértelmű igen/nem választ képes generálni az eldöntendő kérdésekre, továbbá a kérdéseket össze tudjuk vetni a fordított párjukkal és a gold standarddal.
    A válaszainak helyessége másodlagos, de nem elhanyagolható.
    A nyelvi modellek a Hugging Face platformról lesznek kiválasztva.

    A program bemenete tetszőleges sornyi bejegyzés a Word-in-Context adathalmaz \texttt{\allowbreak test} metszetéből (vagy tetszőleges, ezzel megegyező formátumú kérdéshalmazból), és tetszőleges modell a Hugging Face platformról. A program során keletkező hibákat le kell kezelni.
    ...
    Elkészült a keretrendszer, egy Python konzolos program, amely képes tetszőleges Hugging Face modell válaszra bírását, valamint kiértékelését automatizálni.
    Egyetlen kattintással vagy terminál paranccsal képes elvégezni a modell inferenciákat az inputként megadott adathalmazon és elvégezni a nyelvi konzisztencia tesztet, statisztikákat készítve az eredményekből.
...
%A tartalomjegyzék:
    ...
    Az egyik legnépszerűbb ilyen teljesítményteszt a SuperGLUE Benchmark~\cite{wang2020superglue}, amely 8, emberi nyelvi megértést igénylő nagyon nehéz érvelés-orientált feladat elé állítja a nyelvi modelleket.
    A WiC (Word-in-Context) probléma~\cite{pilehvar2019wic} a SuperGlue 8 feladatának egyike.
    ...
    Az AI megoldások - például Hugging Face~\cite{huggingfacehub} nyelvi modellek használatának - ismerete előnyt jelent mind a munkahelyeken, mind a magánéletben.
    ...
    Másrészt közel azonos méretű, de különböző cégek által fejlesztett modellek teljesítménye között is óriási különbség lehet, még akkor is, ha azok azonos célra, pl.\ utasítás-végrehajtásra készültek (\texttt{\allowbreak instruct} modellek).
    Emiatt azt is feltételezem, hogy lesznek különbségek az ilyen hasonló méretű, de eltérő architektúrájú modellek között.
    ...
    Az elmúlt években számtalan, a nagy nyelvi modellek szemantikus konzisztenciájával, illetve logikai stabilitásával kapcsolatos cikk jelent meg, jól mutatva, hogy a téma nagy népszerűségnek örvend.%
    A Google Scholar például 22~100 találatot listáz az \texttt{\allowbreak analyzing the semantical consistency of large language\\ models} kifejezésre, amelyeknek a java részét az elmúlt években publikálták.~\cite{yang-etal-2024-enhancing}.
    ...
    \begin{itemize}
        ...
        \item A szintén a Témavezetőm által ajánlott \texttt{\allowbreak vllm}~\cite{kwon2023efficient} szintén egy aktívan fejlesztett nyílt forráskódú projekt, amely elsősorban a memóriagazdálkodásra fókuszál.
        \item Az \texttt{\allowbreak LM Studio}~\cite{lmstudio} grafikus felület.
        Modellmenedzsment, többszörös modellváltás, helyi inferencia elérhető benne.
        \item \href{https://github.com/oobabooga/text-generation-webui}{text-generation-webui}~\cite{textgen_webui}: webes frontend, backendként pl. \texttt{\allowbreak llama.cpp}, nagyon rugalmas, sokféle modellt és konfigurációt támogat.
        \item A \texttt{\allowbreak GPT4All}~\cite{gpt4all}: kezdeti belépő a helyi LLM-használatba — CPU-barát, alacsony küszöb, egyszerű használat jellemzi.
    \end{itemize}
...
Does the word `w` mean the same thing in `s2` and `s1`?
    ...
    Ez az egyszerű logikai invariancia - a két mondat sorrendjének felcserélése - az ember számára könnyen felismerhető és belátható, hogy a kérdés értelmét nem módosítja, ám a nyelvi modelleket, főleg az alacsony, ``csak'' pár milliárd paraméterszámúakat könnyen összezavarja.
...
% Requires: \usepackage{booktabs}
    ...
    Inputként kap egy promptot, egy legfeljebb \texttt{\allowbreak n} token hosszúságú szöveget, ahol az \texttt{\allowbreak n} függ a modelltől és a paraméterezéstől. Erre fog legfeljebb \texttt{\allowbreak m} hosszúságú válaszban inferenciával, azaz következtetéssel válaszolni. Azért hívják modellnek a modelleket, mert az emberi nyelvet és kommunikációt utánozzák (modellezik).
    ...
    Ahhoz, hogy a nyelvi modelleket ki tudjam értékelni, meg kell ismerni néhány alapfogalmat, koncepciót, amely a \texttt{\allowbreak Hugging Face transformers} könyvtár használata során is elengedhetetlen lesz.
...
%    \vspace*{0.5cm}
    ...
    A projekt kutatási szempontból érdekes része így csak az újraírt \texttt{\allowbreak src/Framework} modulban található.
    A többi nagyrészt archívum és mellőzhető a projekt megértéséhez.
    A \texttt{\allowbreak src/Framework} 3 almodult tartalmaz, amelyek mindegyike egy önálló, de nagyon egyszerűen futtatható programot tesz ki. Egyesítve is lehet futtatni, de több indoka is van, hogy miért inkább egyesével érdemes elindítani a programokat. Az első és utolsó modul offline futtatható és nem erőforrásigényes, a hibák esélye minimális. A középső modul a fő ok, amelyben rengeteg a hibaforrás és a nemdeterminizmus. Online API-hívásokat végez és GPU-t igényel (GPU hiányában pedig hatalmas erőforrásigényt és számítási kapacitást), ráadásul sok harmadik féltől származó függvénykönyvtárra támaszkodik. Emiatt számos hálózati és memóriakezelési hibába futhat, így érdemes külön futtatni ezt a programot, és a hibákat lekezelni. A modulok futtatási sorrendje fontos.
    ...
    Helyette nem forduló, kaotikus, gyakran egymástól teljesen független szkriptek szerepeltek rendezetlenül.
    ...
    Csak a mostani \texttt{\allowbreak ModelInputPreparernek} megegyező rész funkcionalitásával rendelkezett standard Python környezetben.
    A Hugging Face modell inferenciájára (a mostani \texttt{\allowbreak HuggingFaceModelInferencer}~\ref{subsubsec:huggingfacemodelinferencer} modullal megegyező funkcionalitás) egy Jupyter Notebookban került sor, Google Colab környezetben.
    A modell outputjának (kimenetének) feldolgozása ( a mostani\\ \texttt{\allowbreak ModelOutputProcessor}) pedig megint egy harmadik környezetben, Google Apps Scriptben került feldolgozásra.

    Az újraírásnál első lépésem volt, hogy a platformot egyesítettem.
    Most az egész szoftver egy sima, standard Python futtatókörnyezetben fut, egyetlen könnyen megtalálható \texttt{\allowbreak main} fájl futtatásával, ipari standard szerint az src mappában lévő globális belépési ponttal, de egyenként is lehet futtathatni a 3 modult, mindegyikhez tartozván egy \texttt{\allowbreak main.py} fájlban található belépési pont.
    ...
    Helyette parancssorból indítható a program és a \texttt{\allowbreak config} fájlok beállításainak módosításával paraméterezhető.
    ...
    Mindegyik modul egy fő futtatható állományt tartalmaz, egy-egy \texttt{\allowbreak main.py} fájt egy-egy \texttt{\allowbreak main} belépési ponttal.
    ...
    A \texttt{\allowbreak ModelInputPreparer} modul inputjai egy lokális adathalmaz és az elkészíteni kívánt kérdések száma.
    ...
    A ModelOutputProcessor feladata a modell outputból adatkinyerés, majd azt a WiC adathalmazhoz készült gold standarddal összevetve statisztikák készítése.
    Ez a gold standard szakértők által lett hitelesítve, ezt elfogadjuk alapigazságnak a szakdolgozat erejéig és ehhez viszonyítjuk majd a modellek válaszait.


    \section{Fejlesztés}

    \subsection{Globális scope}
    \texttt{\allowbreak GlobalMain.py}: ebben a szkriptben található a fő belépési pont, amely egyszerre mind a három modult lefuttatja.


        \texttt{\allowbreak GlobalData} mappa: Ez a GlobalMain futtatásakor használt ``adatbázis'', adattároló mappa. A modulok egyesével való futtatásakor nem ezt használják, hanem az adott modul gyökerében található \texttt{\allowbreak data} mappát. Mindegyik modulhoz tartozik ebből egy-egy külön.


        \texttt{\allowbreak Data} mappák: az egyes modulokhoz tartozó input és output fájlok tárolását szolgálják.


        \texttt{\allowbreak *.in} fájlok: az input fájlok egységes kiterjesztése. Ha \texttt{\allowbreak *.in} a kiterjesztése, akkor valamelyik fájl azt bemenetként használni fogja.


        \texttt{\allowbreak .out} fájlok: az output fájlok egységes kiterjesztése. Általában ``append'', azaz hozzáfűző módban írat a program beléjük, hogy így meg legyenek őrizve a korábbi futtatások eredményei is.
    ...
    Mindhárom modul futásideje mérve van, a \texttt{\allowbreak time} Python modul \texttt{\allowbreak perf\_counter} algoritmusával mérve. A futásidő konzolos logként tekinthető meg.
    ...
    Ez a \texttt{\allowbreak WiC} adathalmaz \texttt{\allowbreak test} metszetének összes rekordjából létre képes hozni egy-egy kérdést, mind egyenes, mind fordított sorrendben, hogy a modellek konzisztenciáját vizsgáljuk. Mivel 1400 rekordból áll, és rekordonként 2 kérdésünk (egyenes, fordított) van, így jön ki a 2800 kérdés. A létrehozás módja a következő:
    ...
    Ebből nekünk csak a szó, első mondat és a második mondat szükséges, a többivel nem is fogunk foglalkozni.
    
    \paragraph{A kérdések létrehozásának folyamata}
    \subparagraph{}

    \subparagraph{LabelAdder: A WiC beolvasása és a random minták kisorsolása.}
    \subparagraph{}
    A WiC adathalmazt beolvassuk a $LabelAdder$ szkriptben, amely beolvassa a fájlt nyers szövegként, a sorokat tabulátor mentén felbontja, majd eltárolja egy listában.
    Ezután meghívja a  $randomsamples.py$ szkriptet, amely legenerál \texttt{\allowbreak n} darab - a \texttt{\allowbreak config.py}-ban meghatározott mennyiségű indexet. Ezek az indexek lesznek a kérdés-válasz párok azonosítói, én \texttt{\allowbreak GUID}-nak neveztem el őket, amely a ``globálisan egyedi azonosító'' (globally unique identifier) rövidítése . Ezután megnyitjuk a fájlt, majd beolvassuk és minden egyes \texttt{\allowbreak GUID}-adik indexű sorból eltároljuk a szót és a két mondatot egy listában. A GUID értéke mindig 0 és 1399 között van, tehát az értéke pont megegyezik a teszthalmaz hosszával, így nem történhet túlindexelés. 
    
    \subparagraph{WordAndSentencesExtractor}
    \subparagraph{}
    Ezután a listát átadjuk a $WordAndSentencesExtractor$ szkriptnek, amely kinyeri belőlük a szót, az első mondatot és a második mondatot.
    Mivel nekünk csak a szó, első mondat és a második mondat szükséges, ezeket a nyers szövegből listákba helyezzi át, a többi adatot eldobja a $WordAndSentencesExtractor$.

    \subparagraph{}
    \subparagraph{SentenceBuilder: A mintaillesztés}
    \subparagraph{}
    Az alapötlet a WiC-minták kérdéssé konvertálása mögött, hogy a mintákból immár kinyert szó, első mondat és a második mondat sztringeket mintaillesztéssel két előre meghatározott sablonba szúrjuk, amelyeknek egyike az egyenes, másik a fordított kérdést gyártja le.
    \begin{figure}[H]
        \centering
        \includegraphics[width=1\linewidth]{sentence_builders}
        \caption{Jól látható, hogy a WiC-mintákat előre meghatározott mintákba szúrjuk be}
        \label{fig:sentence_builders}
    \end{figure}

    Ezután kétszer végigmegyünk a listán. Először elkészítjük az egyenes kérdéseket, másodjára pedig a fordítottakat. A kérdéseket úgy készítjük el, hogy az angol nyelvű kérdést tartalmazó, úgynevezett Python f-sztringekbe (formázott sztringekbe) helyezzük a szót és a 2 mondatot, lásd \texttt{\allowbreak SentenceBuilder}.
    

    \subparagraph{SentenceNormalizer: A nyelvtani előfeldolgozás}
    \subparagraph{}
    Ugyan csak kényelmi dolog, de a mondatok az adathalmazban tokenizálva vannak, emiatt minden írásjel szóközzel van elválasztva az előtte szereplő tokentől - vagyis pontosabban tabulátorral, de a programomban szóközt használok az egyes részek szeparálására. A lényeg, hogy az AI modellek hatékonyabb prompotolása miatt, és hogy számomra, ember számára is érthetőbb legyen, visszaalakítottam standard, nyelvtanilag helyes angol mondatokká.
    Ehhez csak egyszerűen minden írásjel és az aposztrófok elől el kellett tüntetnem a szóköz(öke)t, amelyhez egy egyszerű sztringcsere műveleteket biztosító algoritmust írtam, lásd \texttt{\allowbreak SentenceNormalizer}. 
    Ezáltal sokkal olvashatóbbá váltak a mondatok.

    Végül a futás eredményét elmentjük egy fájlba, ahonnan majd a következő modul azt elérheti.
    ...
    A négy egymás alatt található szkript egymás után hívódik meg, és mind a run szkriptnek adják vissza az irányítást a befejeztük után.

    \subsection{HuggingFaceModelInferencer}
A második modul a HuggingFaceModelInferencer (modell inferencia végrehajtó).
    Ebben a modulban történik a konkrét modell lefuttatása az előző modulban előállított kérdéseken.

Az előző modulban láthattuk, hogy a kérdések legyártása viszonylag egyszerű fájl- és sztringfeldolgozó algoritmusokkal lehetséges. A modell futtatása a nagyméretű promptom és annak speciális felépítése miatt viszont nem egy triviális feladat. Még kisebb méretű Hugging Face modellek futtatása is rendkívül körülményes. Ez egy internetelérést és rendkívül sok időt és erőforrást (CPU, GPU, RAM) igénylő művelet. Így nem meglepő, hogy ennek a modulnak az elkészítése vette igénybe az idő túlnyomó részét a szoftver elkészítésében.

A program elindítása előtt érdemes a \texttt{\allowbreak config.py} és a \texttt{\allowbreak modelname.py} beállításait ellenőrizni és igény szerint módosítani. A modell tetszőleges lehet, de ajánlott a \texttt{\allowbreak modelname.py} fájlban található \texttt{\allowbreak supported\_models} lista a keretrendszerrel már tesztelt modelleket tartalmazza, ezért célszerű ezek közül választani. A \texttt{\allowbreak config.py} listában lévő modellek közül, annak is az első indexein elhelyezkedő modellek közül választani, mert ezek kompatibilisebbek a keretrendszerrel. A \texttt{\allowbreak config} fájlban lehetőség van a promptnak az utasítás részén változtatni, amely aztán a kérdések elé kerül a prompt-felépítés során. Ebben lehet adni, hogy hány kérdésre válaszoljon, milyen kulcsszavakkal, terjedelemben, stb. A konzolra történő kiíratások mennyiségét is itt lehet állítani.
...
Ezután a program azt is leellenőrzi, hogy tényleg a kérdések fele-e fordított, a most már biztosan páros hosszúságú kérdéshalmazból. Ezt úgy teszi, hogy n kérdés esetén összehasonlítja az 1. és az \texttt{\allowbreak n/2+1}. kérdésben szereplő 1. és 2. mondatot, hogy azok tényleg egymás fordítottjai-e. Hogyha nem, akkor figyelmezteti a felhasználót, hogy hibát talált a kérdéssorban. A felhasználó itt szintén választhat, hogy kilép a programból, vagy továbblép potenciálisan hibás input kérdésekkel.

Ezt követően történik a 3. féltől származó könyvtárak betöltése. A szükséges fő csomagok a \texttt{\allowbreak torch}, \texttt{\allowbreak transformers}, \texttt{\allowbreak accelerate} és \texttt{\allowbreak huggingface\_hub}, de bizonyos modellek további függőségeket is igényelnek. Ha valamelyik hiányzik, a Python futásidőben hibát jelez.
...
A \texttt{\allowbreak torch} csomag \texttt{\allowbreak no\_grad()} kontextuskezelőjével kikapcsoljuk a gradiensek számítását a modell összes műveletére, mivel inferencia során erre nincs szükség. A program további része ebben a kontextusban fut.

Ezután történik a \texttt{\allowbreak transformers} csomag komponenseinek betöltése. Első lépésként a tokenizer létrehozása történik a $AutoTokenizer$ $from\_pretrained()$ metódusával. A tokenizer \texttt{\allowbreak apply\_chat\_template()} metódusa a promptot a kiválasztott modell által elvárt formátumra alakítja. A modell betöltése az $AutoModelForCausalLM$ $from\_pretrained()$ metódusával történik, amely — amennyiben elérhető —\\ \texttt{\allowbreak safetensors} formátumú súlyokat tölt le.

A modell és a tokenizált bemenet áthelyezése a megfelelő eszközre a\\ $model.to(device)$ és $inputs.to(device)$ hívásokkal történik. A válasz generálása a\\ \texttt{\allowbreak generate()} metódussal történik, amely jelen konfigurációban pontosan egy új tokent állít elő\\ $max\_new\_tokens=1$. A kapott tokenhalmazt a \texttt{\allowbreak tokenizer.decode()} segítségével alakítom vissza szöveggé.
...
A futás végén megtörténik a modell válaszainak fájlba írása, továbbá a futás során keletkezett meta-információk is külön fájlokba kerülnek.
    ...
    A harmadik modul a ModelOutputProcessor (modell kimenet feldolgozó).\\
    A \texttt{\allowbreak ModelOutputProcessor} bemenete a HuggingFaceModelInferencerben futtatott modell válaszai.
    Természetesen itt is van validálás a program bemenetére.
    Ha a modell jól válaszolt, akkor \texttt{\allowbreak n} darab kérdés esetén a válaszok száma is \texttt{\allowbreak n} kell, hogy legyen.
    ...
    Tehát a main szkript meghívja a run szkriptet, amely továbbadja az irányítást a \texttt{\allowbreak TernaryClassifier} szkriptnek. A szkript három kategóriába sorolja a válaszokat: ``igen'', ``nem'' és ``talán'' osztályokba.
    Az osztályozás során végigmegy az előző modulból inputként kapott modell válaszokon és soronként eldönti, hogy a ``Yes'', a ``No'', vagy a ``?''  (nem egyértelmű) kategóriába sorolható-e be a válasz. Minden sor pontosan egy kategóriába kerül besorolásra.
    Az eldöntési algoritmusnak több módszere is van, ezért választható itt az \texttt{\allowbreak AnswerAwareClassificationRule} és a \texttt{\allowbreak SentenceClassifier}, amelyet a config fájlban található \texttt{\allowbreak ADAPTIVE\_RUN} változóval lehet beállítani.
    Ezután a kategóriákat a \texttt{\allowbreak TernaryResultsProcessor} összeveti a gold standardban szereplő értékekkel, majd az összevetés eredményei alapján készülnek el a \texttt{\allowbreak run} szkriptben a végeredmények.
    ...
    Különösen igaz ez a \texttt{\allowbreak ModelOutputProcessor}, esetén, hiszen az eredmények könnyen számszerűsíthetők.
    ...
    Ehhez viszont tisztáznunk kell a pontosságfogalmakat.
    Éppen ezért minden végeredményt egy 0 és 100\% közé normalizált számként fejez ki a programom.
    Az eredményként az alábbi statisztikai indikátor mérőszámokat számítja ki a modul:
    \begin{itemize}
        \item Konzisztens válaszpárok aránya
        \item Pontos válaszok aránya
        \item Pontos válaszpárok aránya
        \item Konzisztensen pontos válaszpárok aránya
        \item Bizonytalan válaszpárok aránya
        \item Konzisztensen bizonytalan válaszpárok aránya
    \end{itemize}
    ...
    Ezt 3 kategóriába sorolja: \texttt{\allowbreak Yes}, \texttt{\allowbreak No} és \texttt{\allowbreak ?}, azaz nem egyértelmű, ``talán''.
    ...
    Ez \texttt{\allowbreak n} kérdés esetén \texttt{\allowbreak n/2} összehasonlítást jelent.
    ...
    Megdaja a gold standarddal egyező válaszok számát.
    Mivel a többi metrika páronként, emez pedig egyenként értelmezendő, így nem összevethető a többi metrikával.
    ...
    \subsubsection{Pontos válaszpárok aránya}
    Minden pontos válaszpár \textbf{1} értékkel kerül beszámításra.

    Pontos válaszpárok alatt azt értjük, hogy egy egyenes-fordított kérdéspár legalább egyike megegyezik a gold standarddal.
    Magyarán abban az esetben tekintünk jónak egy teszteset párost, ha az arra kapott válasz nem konzisztensen rossz (tehát legalább az egyik sorrendben helyes a megoldás).
    ...
    Mivel a bizonytalan párok csak páronként értelmezhetőek, így a végeredménybe minden találat \textbf{1} értékkel kerül beszámításra.
    ...
    Mivel a konzisztensen bizonytalan párok csak páronként értelmezhetőek, így a végeredménybe minden találat \textbf{1} értékkel kerül beszámításra.
    ...
    A \texttt{\allowbreak ModelOutputProcessorban} található naplófájl, a logFile.txt tárolja a legutóbbi futtatások eredményeit.
    ...
    Az {adathalmaz sor-kérdés-egyenes válasz-fordított válasz-gold standard sor} \texttt{\allowbreak GUID}-jai \textbf{pontosan megegyeznek-e}. A \texttt{\allowbreak GUID} az eredeti, WiC adathalmazban elfoglalt sorszám, amelyre indexként tekintünk a beolvasás után. A \texttt{\allowbreak GUID} az indices.out fájlban található.
    ...
    Következő lépésként lépjünk a projekt gyökérmappájába. Adjuk ki a \begin{lstlisting}
> & .\.venv\Scripts\Activate.ps1
    \end{lstlisting} parancsot a virtuális környezet aktiválásához. Majd adjuk ki a \begin{lstlisting}
> pip install torch transformers accelerate huggingface_hub
    \end{lstlisting} parancsot a szükséges csomagok telepítéséhez. Ha nagyobb modelleket is szeretnénk futtatni, akkor más csomagokra is szükség lehet. Ez esetben a 
    \begin{lstlisting}
> pip install torch transformers accelerate huggingface_hub
   hf_xetm optimum
    \end{lstlisting} az ajánlott parancs.

    Itt kétféle opciónk van. Egyszerű mindhárom modult lefuttató futtatásért adjuk ki a gyökérkönyvtárban - ahol a \texttt{\allowbreak /.git/} és az \texttt{\allowbreak /src/} mappa van - a
    \begin{lstlisting}
...
> py-m src.Framework.ModelOutputProcessor.main
    ...
    A paraméterezés helyette a \texttt{\allowbreak config} és a \texttt{\allowbreak .in} kiterjesztésű fájlokban történik.
...
> py-3.13 -m 
    src.Framework.HuggingFaceModelInferencer.main

> py-3.13 -m src.Framework.ModelOutputProcessor.main
    \end{lstlisting}

    Windowson szükség lehet a következő parancs kiadására ahhoz,
    hogy az operációs rendszer engedélyezze a Python szkriptek futtatását:
...
%    \end{figure}
    ...
    Ez duplikált importálásnak tűnhet, ám nem az, hiszen a vezérlési szerkezet biztosítja, hogy ha az első elérési út megtalálása meghiúsul, akkor a másik útvonalról töltődjenek be a szükséges modulok.
...
\chapterwithdesc{Kísérlet}
    {Négy konkrét modell kiértékelése véletlenszerű mintahalmazon a keretrendszerben}
    A végső cél, amiért az egész szoftver keretrendszer készült, az volt, hogy legalább két, de inkább több modellt összehasonlíthassunk minél nagyobb mennyiségű kérdésen.
    ...
    \begin{itemize}
        ...
        \item Minden modell temperature, top-p és top-k értéke az alapértelmezett,
        \item Az utasítás minden kérdés esetén a következő: \texttt{\allowbreak Answer the question with just a single `Yes` or `No`.} (Válaszolj a kérdésre csak egyetlen ``Igen'' vagy ``Nem'' szóval.),
        \item Ha a 4 adat, az egyenes válasz, a fordított válasz, az eredeti WiC-adathalmazbeli sor, az ehhez tartozó gold standard címke \texttt{GUID-ja} valamelyike nem egyezik a többivel, akkor hiba csúszott a rendszerbe, és újra kell kezdeni a legkésőbbi helyes állapottól.
    \end{itemize}
...
%    Mind a hármat közepes és nehéz kérdéseken, mindkét kérdésnehézséget egyenes és fordított sorrendben teszteltem. Ez így összesen \texttt{\allowbreak 3*3*2=18} kiértékelést jelentett.
    Hangsúlyozom, hogy nem a {gemma-2b-it}, hanem az újabb {gemma-\textit{2}-2b-it} változatot választottam.
%    A modellek eredményeit összessítő táblázat a 4.1. táblázatban látható.\ref{tab:model-eval-summary}

    \section{Négy modell összehasonlítása 100 véletlenszerű kérdésen}
    A keretrendszer éles próbájaként a $Qwen/Qwen2.5-0.5B-Instruct$, a\\ $Qwen/Qwen2.5-1.5B-Instruct$ és a $Google/Gemma-2-2b-it$ modelleket hasonlítottam össze. 
    Az előbbi kettő az Alibaba Cloud gyártmánya, míg utóbbi a Google szüleménye. A választással megfigyelhetjük négy eltérő méretű (0.5B vs 1.5B vs 2B) és két eltérő gyártójú, ezáltal architektúrájukban is némileg különböző \textit{instruct} modell különbségét. 

    Mind a 600 választ egyesével generáltattam le, majd egyenként leellenőriztem, hogy a 4 adat, az egyenes válasz, a fordított válasz, az eredeti WiC-adathalmazbeli sor, az ehhez tartozó gold standard címke \texttt{GUID-ja} valóban megegyezik-e, felhasználva a \texttt{\allowbreak ModelOutputProcessor} modulom indexereit, így ügyelve a módszertan tisztaságára.

    Keretrendszerem érdeme, hogy végül sikerült a négy modell válaszadását 100 véletlenszerű kérdésre (és a fordított párjukra) hiánytalanul elérni.
    A modellek sikeresen csak ``Yes'' és ``No'' válaszokat adtak, amely miatt az automatikusan 0 értékű ``konzisztensen bizonytalan'' mérőszámot nem tüntetem fel, és a ``bizonytalan'' mutatót is csak vizuális kiemelésképpen.
...
\begin{tikzpicture}
    \begin{axis}[
        title={Önmagával és az elvárt címkékkel részben vagy egészben megegyező válaszpárok % ide kéne 1 sortörés
        eloszlása modellenként},
        ...
        ylabel={Darabszám},
        symbolic x coords={
            Önmagával konzisztens párok,
            Részben pontos párok,
            Teljesen pontos párok
        },
        xtick=data,
        x tick label style={
            rotate=45,
    ...
    \addplot[
        ...
        (Önmagával konzisztens párok, 94)
        (Részben pontos párok, 71)
        (Teljesen pontos párok, 65)
    };
    ...
    \addplot[
        ...
        (Önmagával konzisztens párok, 79)
        (Részben pontos párok, 76)
        (Teljesen pontos párok, 55)
    };
    ...
    \addplot[
        ...
        (Önmagával konzisztens párok, 73)
        (Részben pontos párok, 76)
        (Teljesen pontos párok, 49)
    };
    ...
    \addplot[
        ...
        (Önmagával konzisztens párok, 82)
        (Részben pontos párok, 83)
        (Teljesen pontos párok, 65)
    };
...
\caption{
Négy modell konzisztens, pontos és bizonytalan párjainak száma a 100 kérdéspáron. 
A konzisztensen pontos párok száma a konzisztens és pontos párok matematikai metszete.
}
...
\begin{tikzpicture}
    \begin{axis}[
        title={Az elvárttal megegyező egyenkénti válaszok száma modellenként},
        ...
        height=4.2cm,
        ylabel={Darabszám},
        symbolic x coords={Pontos egyenkénti válaszok},
        xtick=data,
        x tick label style={
            rotate=0, 
            anchor=north,
    ...
    \addplot[
        ...
        (Pontos egyenkénti válaszok, 136.00)
    };
    ...
    \addplot[
        ...
        (Pontos egyenkénti válaszok, 131.00)
    };
    ...
    \addplot[
        ...
        (Pontos egyenkénti válaszok, 125.00)
    };
    ...
    postaction={pattern=dots, pattern color=purple!40!black}
        ...
        (Pontos egyenkénti válaszok, 148.00)
    };
...
\caption{Négy modell pontos egyenkénti válaszainak (gold standard címkékkel való egyezések számának) összehasonlítása ugyanezen a 200 véletlenszerű kérdésen.}
...
\begin{tikzpicture}
    \begin{axis}[
        title={Az elvárttal megegyező válaszok száma modellenként},
        title style={font=\large, yshift=6pt},
        ybar,
        bar width=0.56cm,
        width=0.7\textwidth,
...
\caption{A négy modell ``Yes'' és ``No'' válaszainak darabszáma ugyanerre a 200 egyenkénti kérdésre összevetve a gold standard címkék arányával. Mivel egy kérdésre az egyenes és a fordított válasznak is ugyanaz a gold címkéje, így az ábrához a ``True'' és ``False'' címkék darabszámát, amelyek megfeleltethetőek a model által adott ``igen'' és ``nem'' válaszoknak, duplán kell számolni.}
...
\begin{tikzpicture}
    \begin{axis}[
        title={A generált címkék különbözete az elvárttól},
        ybar,
        bar width=0.8cm,
        width=0.7\textwidth,
        height=5cm,
        ...
        nodes near coords,          % Ez jeleníti meg az értékeket a sávok felett
        nodes near coords align={vertical}, % függőlegesen középre igazít
        legend style={
            at={(0.5,-0.25)},
            anchor=north,
            legend columns=4,
    ...
    \end{axis}
\end{tikzpicture}
\caption{A  négy modell ``Yes''-``No'' válaszainak eloszlásának különbözete (különbségének abszolút értéke) az elvárt eloszláshoz képest.
}
\label{fig:YesNoDeviation}
\end{figure}

Az ábra az elvárt eloszlástól való szimmetrikus eltérést mutatja - annak abszolút értékét. Az ábrán még szebben látszik, hogy a Qwen modellek milyen nagy mértékben eltérnek a címkék elvárt eloszlásától, míg a nagyobb modelleknek sikerült jobban eltalálniuk az arányt.
A legkisebb modell 58-szor többször tippelte az egyik címkét a kelleténél, míg a legnagyobb modell csak 20-szor. Alapul véve ezt, és a pontosságaik arányát, elmondható, hogy a nagyobb modell teljesítménye megbízhatóbb.
Inkább az eltérés mérete lényeges és érdekes. Annak előjele másodlagos, így azzal nem foglalkozunk.

    \section{Kísérlet jellemzői}
...
\begin{itemize}
    ...
    \section{Jövőbeli elgondolások a kísérlet továbbfejlesztésére}
    A jelenleg elvégzett kísérletre vonatkozóan több ötletem is adótott a kutatás továbbmélyítésére és megbízhatóbbá tételére.

    Az aktuális 100 kérdéspárból álló minta statisztikailag nem elegendő ahhoz, hogy szignifikáns következtetéseket lehessen levonni a modellek közötti teljesítménykülönbségekről.
    Ha mindenképpen a jelenlegi 100 párból álló mintából szeretnénk kimutatni az összefüggést a modell növekvő paraméterszáma és a javuló teljesítménye között, akkor azt lenne értemes összeszámítani, hogy melyik modell válaszol többször konzisztensen helyesen az alulreprezentált - esetünkben az ``igen'' - oldalra. 
    Érdekes lenne a ``konzisztensen helyes'' csoport kettébontása és az így kapott konzisztensen pontos párok súlyozása is az alapján, hogy mennyivel nagyobb az adott csoport a másiknál. 
    Ám ennél valószínűsíthetően jobb módszer lenne újabb kérdéspárok hozzáadása a mintához. Segíthet teljesebb képet kapni továbbá újabb modellek bevonása, például a Phi-4-mini-instruct tesztelése, amellyel kompatibilis a keretrendszer, de a kiértékelése nem készült el teljeskörűen.
    Az inferencia során többszörös futtatásokat lehetne végezni minden modellel (minimum 3-5 futtatás), hogy az véletlenségből adódó szórást meg lehessen határozni. Az eredmények átlagolásával még megbízhatóbb becsléseket kapnánk a modellek valódi teljesítményére.


\chapter{Konklúzió} % TODO
Mivel az értékelés csupán 200 egyedi kérdésen, vagyis 100 páron történt, így a következtetések csak korlátozottan általánosíthatóak és nem tekinthetjük teljesen reprezentatívnak.
Azonban ebből a méretű adatból is érdekes megfigyeléseket lehet levonni. 
Az ábrákból leolvasható, hogy a Qwen2.5-0.5B-Instruct és a Qwen2.5-1.5B-Instruct modell is jelentősen előnyben részesíti a ``nem'' választ az ``igen'' válaszaihoz képest (0.5B: 188 darab, 1.5B: 179 darab), míg a \texttt{\allowbreak Gemma-2-2b-it} 
modell jóval kiegyensúlyozottabb eloszlást mutat (81 ``igen'' és 119 ``nem'' válasz). Tény, hogy a gold standard címkék eloszlása is torzított volt, a ``nem'' címkék aránya jelentősen felül volt reprezentálva (130 ``nem'' és csupán 70 ``igen'' címke), ám a Qwen-modellek válaszaránya még így is jelentősen eltért a várttól, különösen a kisebb, 0.5B modell esetén.
A jelenség fő oka valószínűleg az, hogy a kisebb méretű Qwen-modelleknek nincs elegendő 
lexikális tudása a WiC-adathalmazra jellemző egzotikus, speciális lexémáknak a magas szintű szemantikus 
reprezentációjához, így biztonsági okokból a ``nem ugyanazt jelentik'' predikció felé 
hajlanak.

A páronkénti pontosság vizsgálata ugyanakkor rávilágít, hogy mégiscsak várhatunk teljesítményjavulást nagyobb paraméterszámú modell választásával: míg az 1.5B modell válasza
76 pár esetében egyezett legalább félig a gold standarddal, addig a 0.5B modell esetén 71, a Gemma esetén pedig 76, vagyis a 
nagyobb Qwen-modell és a Gemma modell közel azonos teljesítményt mutatott ezen a téren, enyhén megelőzve a félmilliárd paraméteres modellt.

A \texttt{\allowbreak Gemma-2-2b-it} modell az arányos válaszadás tekintetében sokkal stabilabbnak 
bizonyult, mint a \texttt{\allowbreak Qwen} modellek, mivel nem torzult el egyetlen válaszkategória irányába sem, ami arra 
utal, hogy a Google által alkalmazott tanítási eljárás jobb általánosító képességet 
eredményez a bináris szemantikai döntési feladatokon.

A konzisztensen pontos párok számában szintén megfigyelhető a különbség: 
a 0.5B modell 65 egyezést adott, míg az 1.5B modell 55-öt, a Gemma pedig 49-et. 
Ez elsősorban arra vezethető vissza, hogy a legkisebb Qwen-modell – a válaszainak erős ``nem''-túlsúlya 
miatt – szinte minden ``nem'' címkét eltalált a gold standardból. A jelenség rávilágít arra, hogy a magas konzisztencia és ``konzisztens helyesség'' mutató nem feltétlenül jelenti a szemantikai megértés tényleges 
javulását, mivel a következetes negatív válaszadás torzítja a ``konzisztensen pontos'' mérőszámot. Bár ez a stratégia statisztikailag kedvező konzisztens viselkedést eredményez, nem szolgáltat bizonyítékot arra, hogy a modell összetettebb, jelentésérzékeny feladatok esetén is következetes és megalapozott döntéshozatalra képes.

A modell-válaszok semleges (``bizonytalan'') kategóriába esése az éles kísérleten egyik modellnél sem 
fordult elő.

Említésre méltó az inferenciák időkülönbsége is a négy modell között, amelyben viszont jelentősen nyert a $Qwen2.5-0.5B-Instruct$. A $Qwen$ félmilliárd paraméteres modellje esetén átlagosan 6 másodpercet vett igénybe egy futtatás 1 darab kérdésre az inferenciával együtt, a másfélmilliárd paraméteres modellje esetén már 16-30 másodpercet, míg a Gemma modell esetén az átlagos válaszidő kb. 35(!) másodperc volt. Megfigyelhető tehát, hogy a méret növekedésével az inferencia futásideje szuperlineárisan növekszik.

A hipotézisekre reagálva ugyan elmondható egyrészt, hogy a paraméterszám növekedésével a modellek páronkénti pontossága javul, ám a többi metrikában nem volt megfigyelhető egyértelmű javulás, sőt enyhe teljesítményromlás volt jellemző. 
Másrészt a nagyobb Qwen modell paraméterszáma ugyan nem különbözik nagyban a Gemma modellétől, ám mégis jelentős eltérések mutatkoznak a válaszok eloszlásában és a konzisztens viselkedésben, amely arra utal, hogy a modell architektúrája és a tanítási eljárás is lényeges hatással van a teljesítményre, nem csupán a paraméterszám.

Sikerült olyan mértékben automatizálnom a keretrendszert, hogy az elkészült szoftver jelentősen megkönnyítette a végső kísérlet elvégzését és így a saját munkámat. Arról nem is beszélve, hogy a keretrendszer nyomonkövethetővé és jól ellenőrizhetővé teszi az eredményeket. A munkafolyamatok túlnyomó többségét sikerült teljesen automatizálnom, megelőzve a manuális kiértékelést és ellenőrzést.
Ha az elkészült munkámmal legalább egy embernek segítettem, akkor már megérte elkészítenem. Bízom benne, hogy a keretrendszerem és a kísérletem hasznos lesz a jövőben mások számára is.
...
%nem kézzel helyettesítendõk:
    ...
    Ezúton szeretnék köszönetet mondani a családomnak, barátaimnak, ismerőseimnek, tanáraimnak és egyetemi tanáraimnak, akik végigkísértek utamon és támogattak tanulmányaim alatt.
    ...
    \begin{itemize}
        ...
        \item A nyelvi modellek tesztelésének és kiértékelésének régi eredményei ebben a
