...
%    Összességében elmondható, hogy a Gemma teljesített a legjobban a három modell közül. Pontos, nem preferálja aránytalanul az egyik osztályt, öntudatos, figyel a saját korábbi válaszaira és konzisztens is, azaz el tud rugaszkodni a szavak sorrendjétől, absztraktabb módon a feladaton tartja a fókuszt. Általános célú feladatokra, például egy programozási feladat megoldására sokkal rosszabbul teljesít, mint egy GPT-4o, Claude 3.7 Sonnet, Grok vagy DeepSeek modelljei. A gemma-2-2b-it nem általános célú modell, hanem elsősorban utasításkövetésre lett optimalizálva.
    ...
    A projekt első verziója kritikákat kapott, ugyanis nehezen átlátható és nagyban AI generált, pongyola minőségű kódra alapszik, így 2025 nyarán újrakezdtem a fejlesztést. Az új szoftvernek 2 fő fejlesztési szempontja volt.
    ...
    A program elindítása előtt érdemes a \verb|config.py| és a \verb|modelname.py| beállításait ellenőrizni és igény szerint módosítani. A modell tetszőleges lehet, de ajánlott a \verb|modelname.py| fájlban található \verb|supported\_models| listában lévő modellek közül, annak is az első indexein elhelyezkedő modellek közül választani, mert ezek kompatibilisebbek a keretrendszerrel. A config fájlban lehetőség van a promptnak az utasítás részén változtatni, amely aztán a kérdések elé kerül a prompt-felépítés során. Ebben lehet adni, hogy hány kérdésre válaszoljon, milyen kulcsszavakkal, terjedelemben, stb. A konzolra történő kiíratások mennyiségét is itt lehet állítani.
    ...
    Az első szükséges csomag a torch, amellyel előkészítjük a hardvert a nagyméretű adat számolására. A torch csomag no\_grad() kontextuskezelő metódusa kikapcsolja a gradiensek számolását a modell összes műveletére. A program maradék részét ebbe a kontextusba helyezzük, mert erre egész biztosan nem lesz szükség.
    Ezután a transformers csomag importálása történik. Ezután következik a modell-specifikus rész, hiszen a különböző modelleknek eltérő az inferencia-végrehajtó logikája. Az inferencia történhet determinisztikusan és nem-determinisztikusan, ezt is a config fájlban lehet állítani. Az alap logika viszont minden modell esetén ugyanaz, főleg mivel a generálási stratégiám részeként minden inferencia eredménye pontosan 1 darab token. Első lépés a választott modell súlyainak letöltése textit{safetensors} formátumban az AutoModelForCausalLM osztály from\_pretrained metódusával. Ezután modell letöltése tokenizálás, történik amelyhez segítséget nyújt a \texttt{transformers} \texttt{AutoTokenizer} osztálya, és annak from\_pretrained metódusa, amely a promptunkat a modell számára érthető tokenekké bontja. Ezután a tokenizer osztály apply_chat_template metódusával a promptot a modell által elvárt formátumra alakítjuk át. Majd kiválasztjuk a céleszközt, a programom esetében a saját hardverem a transformers .to(model.device). Végül megtörténik a válasz legyártása a modell generate metódusával, amely a tokenizált promptot bemenetként megkapva legyártja a választ tokenek formájában. Ezt a tokeneket a tokenizer decode metódusával visszaalakítjuk szöveggé.
